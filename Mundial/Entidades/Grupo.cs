...
namespace Entidades
{
    ...
    public class Grupo 
    {
        ...
        private short maxCantidad = Torneo.MAX_EQUIPOS_GRUPO;
        ...
        public Grupo Leer()
        {
            try
            {
                string consulta = "SELECT id, nombre, grupo FROM Equipos";
                GrupoDao.Comando.CommandText = consulta;
                GrupoDao.Conexion.Open();
                SqlDataReader oDr = GrupoDao.Comando.ExecuteReader();
                while(oDr.Read())
                {
                    if(this.grupo.ToString() == oDr["grupo"].ToString())
                    {
                        this.equipos.Add(new Equipo(int.Parse(oDr["id"].ToString()), oDr["nombre"].ToString()));
                    }
                }
            }
            catch (Exception e)

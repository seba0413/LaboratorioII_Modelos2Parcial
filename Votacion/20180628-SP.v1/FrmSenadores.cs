        ...
        public void ManejadorVoto(string senador, Votacion.EVoto voto)
        {
            ...
            else
            {
                ...
                if (aux == 0)
                {
                    ...
                    votacion.ContadorAfirmativo = short.Parse(lblAfirmativoLabel.Text);
                    votacion.ContadorNegativo = short.Parse(lblNegativoLabel.Text);
                    votacion.ContadorAbstencion = short.Parse(lblAbstencionesLabel.Text);

                    Dao guardarEnBaseDeDatos = new Dao();
                    guardarEnBaseDeDatos.Guardar("", votacion);

                    SerializarXML<Votacion> serializarXML = new SerializarXML<Votacion>();
                    serializarXML.Guardar("ResultadosVotacion.xml", votacion);
